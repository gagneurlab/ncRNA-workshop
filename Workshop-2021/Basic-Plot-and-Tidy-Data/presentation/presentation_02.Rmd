...
```{r echo=F, fig.show = "hold", out.width = "100%", fig.align = "center", fig.pos='H'}
knitr::include_graphics("../../assets/img/tidy-1_alternate.png")
```
...
Data from 2016 US presidential vote^[https://www.kaggle.com/stevepalley/2016uspresidentialvotebycounty?select=pres16results.csv] is an example of a tidy dataset:
...
<!-- In the table below, what do the values in this table represent? We can see that there is a value of 745 observed in Afghanistan in 1999, but what does it refer to exactly? -->

<!-- ```{r} -->
<!-- as.data.table(table4a) -->
<!-- ``` -->

<!-- Here the column headers 1999 and 2000 are values of the variable *year*. -->
...
Untidy:
```{r echo=F, fig.show = "hold", out.width = "80%", fig.align = "center", fig.pos='H'}
knitr::include_graphics("../../assets/img/untidy_rel.png")
```
...
knitr::include_graphics("../../assets/img/tidy_rel.png")
```

## Multiple variables are stored in one column

This table is an example of multiple variables are stored in one column.
Untidy:
```{r, echo=FALSE}
table3
```
...
Untidy:
```{r echo=F, fig.show = "hold", out.width = "100%", fig.align = "center", fig.pos='H'}
knitr::include_graphics("../../assets/img/untidy_weather.png")
```
...
```{r, echo = FALSE}
t1 <- tidyr::table1
countries_list <- split(t1, t1$country) %>% lapply(. %>% subset(select = -country) %>% as.data.table)
countries_list
```
...
```{r, echo=F}
mtcars_dt <- data.table(model = row.names(mtcars), as.data.table(mtcars))
head(mtcars_dt[,c ("model", "mpg", "cyl", "hp", "wt")])
```
...
```{r echo=F, fig.show = "hold", out.width = "60%", fig.align = "center", fig.pos='H'}
knitr::include_graphics("../../assets/img/tidy-gather_alternate.png")
```

Melting the previous table can be achieved by using the __data.table__ function `melt()`:
...
knitr::include_graphics("../../assets/img/tidy-spread_alternate.png")
```
...
We split up the `proportion` column into two columns, one containing the votes and the other one containing the total votes with the function `separate()` from the `tidyr` package. By default, columns are separated by any non-alphanumerical character (such as ",", ";", "/",...):
...
<!-- From these tables, which one is in tidy form? -->
...
<!-- 4. `table4 <- rbind(table4a, table4b)` -->

<!-- ## Answer -->

<!-- From the tables of the previous slide, which one is in tidy form? -->
...
```{r echo=F, fig.show = "hold", out.width = "80%", fig.align = "center", fig.pos='H'}
knitr::include_graphics("../../assets/img/untidy_rel_crop.png")
```
...
```{r echo=F, fig.show = "hold", out.width = "80%", fig.align = "center", fig.pos='H'}
knitr::include_graphics("../../assets/img/untidy_weather_crop.png")
```
...
4. melt, unite and cast

## Quiz: What transformations are required to tidy the following data?

**Answer:** 4. melt, unite and cast. Tidy form:

```{r echo=F, fig.show = "hold", out.width = "50%", fig.align = "center", fig.pos='H'}
knitr::include_graphics("../../assets/img/tidy_weather.png")
```


<!-- ## Advanced: Columns containing sets of values -->

<!-- Below are a few columns of the answers from the 2017 survey conducted by Kaggle. In those columns, we observe another type of untidy data. In this survey, multiple choice questions were asked from which multiple answers could be selected. For each individual the selected answers are concatenated into a string. -->

<!-- ```{r, echo=F} -->
<!-- options(width = 60) -->
<!-- survey <- fread('../../dataviz_book/extdata/kaggle-survey-2017/multipleChoiceResponses.csv') -->
<!-- ``` -->

<!-- ```{r} -->
<!-- survey[, .(LanguageRecommendationSelect, LearningPlatformSelect,  -->
<!--            PastJobTitlesSelect)] %>% head(n=3) -->
<!-- ``` -->


<!-- ## Advanced: Columns containing sets of values -->

<!-- Below is one solution, of how the `LearningPlattformSelect` column could be transformed into a tidy format. -->
...
1. Get all file names of the directory into a list called `files`:
...
2. load all file contents with `fread` using `lapply`
```{r}
...
```{r out.width = "250px", out.height = "60px", echo=FALSE, fig.align='center'}
knitr::include_graphics("../../assets/img/Joins_Diagram.png")
```

^[https://documentation.mindsphere.io/resources/html/predictive-learning/en-US/Types_of_Joins.htm]
...
```{r echo=F, fig.show = "hold", out.width = "50%", fig.align = "center", fig.pos='H'}
knitr::include_graphics("../../assets/img/right-join.png")
```
...
* R. Fisher (1936). See <https://en.wikipedia.org/wiki/Iris_flower_data_set>

```{r echo=F, fig.show = "hold", out.width = "40%", fig.align = "center", fig.pos='H'}
knitr::include_graphics("../../assets/img/iris.png")
```
...
Here each row represents one flower.
Tidy:
```{r, echo = T, results = T}
# Iris dataset, usual representation
iris_dt[1:3,]
```
Here each row represents one measurement.
Also tidy:
```{r, echo = T, results = T}
...
```{r echo=F, fig.show = "hold", out.width = "50%", fig.align = "center", fig.pos='H'}
knitr::include_graphics("../../assets/img/sample_database.png")
```
...
```{r, echo = F, results = T}
library(data.table)
prod_dt <- data.table(productCode = c("p018", "p030", "p018"), productName = c("dryer", "phone", "dryer"), customerNumber = c("c001", "c001", "c002"), customerName = c("Smith", "Smith", "Lewis"), priceEach = c(450, 600, 450), state = c("CA", "CA", "AZ"), quantOrdered = c(1,2,1))
prod_dt[, .(productCode, quantOrdered, priceEach, customerNumber, customerName, state)]
```
...
Back-end
```{r echo=F, fig.show = "hold", out.width = "90%", fig.align = "default", fig.pos='H'}
knitr::include_graphics("../../assets/img/cutlery_glasses.png")
```
...
Front-end
```{r echo=F, fig.show = "hold", out.width = "90%", fig.align = "default", fig.pos='H'}
knitr::include_graphics("../../assets/img/dinner.jpg")
```
